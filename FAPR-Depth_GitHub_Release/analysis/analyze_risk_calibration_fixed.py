...
def save_qualitative(record: Dict[str, Any], path: Path, title: str) -> None:
    ...
    gain_values = np.abs(record["true_gain"][support]) if np.any(support) else np.abs(record["true_gain"])
    gain_max = float(np.percentile(gain_values, 98)) if gain_values.size else 1.0e-4
    gain_max = max(gain_max, 1.0e-4)

    proposal_values = np.abs(record["proposal_update"][support]) if np.any(support) else np.abs(record["proposal_update"])
    accepted_values = np.abs(record["accepted_update"][support]) if np.any(support) else np.abs(record["accepted_update"])
    proposal_max = float(np.percentile(proposal_values, 99)) if proposal_values.size else 1.0e-4
    accepted_max = float(np.percentile(accepted_values, 99)) if accepted_values.size else 1.0e-4
    proposal_max = max(proposal_max, 1.0e-4)
    accepted_max = max(accepted_max, 1.0e-4)

    bg_color = "#cfcfcf"

    def masked_diverging(image: np.ndarray, active_mask: np.ndarray | None = None) -> np.ma.MaskedArray:
        mask = ~support if np.any(support) else np.zeros_like(image, dtype=bool)
        if active_mask is not None:
            mask = mask | (~active_mask)
        return np.ma.masked_where(mask, image)

    def cmap_with_bg(name: str):
        cmap = cm.get_cmap(name).copy()
        cmap.set_bad(color=bg_color)
        return cmap

    predicted_gain_mask = support & (np.abs(record["predicted_gain"]) > max(gain_max * 0.03, 1.0e-6))
    true_gain_mask = support & (np.abs(record["true_gain"]) > max(gain_max * 0.03, 1.0e-6))
    proposal_mask = support & (np.abs(record["proposal_update"]) > max(proposal_max * 0.03, 1.0e-6))
    accepted_mask = support & (np.abs(record["accepted_update"]) > max(accepted_max * 0.03, 1.0e-6))

    panels = [
        (record["rgb"], "RGB", None, None, None, False),
        (record["gt"], "Ground truth", "viridis", vmin, vmax, False),
        (record["safe"], "Safe posterior", "viridis", vmin, vmax, False),
        (record["candidate"], "Candidate", "viridis", vmin, vmax, False),
        (record["final"], "Risk-accepted output", "viridis", vmin, vmax, False),
        (record["safe_error"], "Safe error", "magma", 0.0, error_max, False),
        (record["candidate_error"], "Candidate error", "magma", 0.0, error_max, False),
        (masked_diverging(record["predicted_gain"], predicted_gain_mask), "Predicted gain", cmap_with_bg("coolwarm"), -gain_max, gain_max, True),
        (masked_diverging(record["true_gain"], true_gain_mask), "True gain", cmap_with_bg("coolwarm"), -gain_max, gain_max, True),
        (record["acceptance"], "Acceptance", "viridis", 0.0, 1.0, False),
        (masked_diverging(record["proposal_update"], proposal_mask), "Proposal update", cmap_with_bg("coolwarm"), -proposal_max, proposal_max, True),
        (masked_diverging(record["accepted_update"], accepted_mask), "Accepted update", cmap_with_bg("coolwarm"), -accepted_max, accepted_max, True),
    ]

    fig, axes = plt.subplots(3, 4, figsize=(16, 11), facecolor="white")
    for ax, (image, name, cmap, lo, hi, use_gray_bg) in zip(axes.flat, panels):
        ax.set_facecolor(bg_color if use_gray_bg else "white")
        if cmap is None:
            ax.imshow(image)
        else:
